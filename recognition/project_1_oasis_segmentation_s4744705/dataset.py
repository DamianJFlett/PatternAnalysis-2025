...
class OASISProjectDataset(Dataset):
    """
    Dataset class specific to the Oasis Dataset. Will work (with paramter changes, potentially)
    for different algorithms/architecture on the same data, but will hopefully just need to change this out if 
    changing to a different dataset with the same architecture.
    """
    def __init__(self, img_directory: str, mask_directory: str):
        """
        Initialises an OasisProjectDataset Object. 
        img_directory: the directory where the origin images are located
        mask_directory: the directory where the masks of those images are located
        ...
        if len(self.mask_names) != self.len:
            raise ValueError("Image and mask sets were not the same length!")
